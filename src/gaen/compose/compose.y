...
%token IF SWITCH CASE DEFAULT FOR WHILE DO BREAK RETURN COMPONENT COMPONENTS UPDATE INPUT_ ANY NONE USING AS CONST_ SELF CREATOR PRE POST VALUE RENDERER SOURCE
%right ELSE THEN

%right <pAst> '=' ADD_ASSIGN SUB_ASSIGN MUL_ASSIGN DIV_ASSIGN MOD_ASSIGN LSHIFT_ASSIGN RSHIFT_ASSIGN AND_ASSIGN XOR_ASSIGN OR_ASSIGN TRANSFORM READY PARENT VISIBLE
...
prop_init
    ...
    | VISIBLE '=' expr     { $$ = ast_create_visible_init($3, pParseData); }
    ;
...
expr
    ...
    | SELF       { $$ = ast_create(kAST_Self, pParseData); }
    | CREATOR    { $$ = ast_create(kAST_Creator, pParseData); }
    | SOURCE     { $$ = ast_create(kAST_Source, pParseData); }
    ;
