...
class ScriptInfo(object):
    def __init__(self, cmpFullPath, includes):
        ...
        self.cppFullPath = ARGS['output_dir'] + cppPath
...
def license_text(comment_chars, file_path):
    lic_path = posixpath.join(ARGS['scripts_dir'], 'license.txt')
...
def registration_cpp_path():
    return posixpath.join(ARGS['output_dir'], "registration.cpp")
...
def cmakeify_bin_script_path(p):
    if ARGS['output_dir'] in p:
        return p.replace(ARGS['output_dir'], '  ${CMAKE_CURRENT_BINARY_DIR}')
    return p

def cmakeify_src_script_path(p):
    if ARGS['scripts_dir'] in p:
        return p.replace(ARGS['scripts_dir'], '  ${scripts_dir}')
    return p
...
def write_cmake(cmp_files, cpp_files, h_files):
    ...
    cmake_path = posixpath.join(ARGS['output_dir'], 'codegen.cmake')
    template = CMAKE_TEMPLATE
    template = template.replace('<<scripts_dir>>', ARGS['scripts_dir'])
...
def main():
    ARGS['scripts_dir'] = sys.argv[1].replace('\\', '/')
    ARGS['output_dir'] = sys.argv[2].replace('\\', '/')
    ...
    source_files = []
    find_source_files(ARGS['scripts_dir'], source_files)
